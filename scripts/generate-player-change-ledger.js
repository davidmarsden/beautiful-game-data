...
if (await exists(previousPath)) {
  const previousRaw = JSON.parse(await readFile(previousPath, "utf8"));
  previous = Array.isArray(previousRaw) ? previousRaw : previousRaw.players || [];
}
...
for (const row of current) {
  ...
  if (num(before.tbg_rating) !== num(row.tbg_rating)) changes.push({ type: "rating_change", player_id: id, player_name: nameOf(row), before: num(before.tbg_rating), after: num(row.tbg_rating), delta: num(row.tbg_rating) - num(before.tbg_rating), current: row });
  if (clubOf(before) !== clubOf(row)) changes.push({ type: "club_change", player_id: id, player_name: nameOf(row), before: clubOf(before) || "Unsigned", after: clubOf(row) || "Unsigned", current: row });
  if (!unsigned(before) && unsigned(row)) changes.push({ type: "newly_unsigned", player_id: id, player_name: nameOf(row), before: clubOf(before) || "Assigned", after: "Unsigned", current: row });
}
...
const summary = {
  generated_at: new Date().toISOString(), previous_players: previous.length, current_players: current.length, first_edition: previous.length === 0,
...
for (const change of changes) {
  const detail = change.type === "rating_change" ? `${change.before} → ${change.after} (${change.delta > 0 ? "+" : ""}${change.delta})` : change.type === "club_change" ? `${change.before} → ${change.after}` : change.type === "newly_unsigned" ? `${change.before} → Unsigned` : change.type === "new_player" ? `added at ${clubOf(change.current) || "Unsigned"}` : "removed from current edition";
