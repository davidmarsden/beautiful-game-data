...
if (!args.pack || !args.targets) {
  console.error("Usage: node scripts/train-smw-rating-model.js --pack=<league-pack.json> --targets=<smw-ratings.csv> [--output=calibration/smw-rating-model.json]");
  process.exit(1);
}

const output = args.output ?? "calibration/smw-rating-model.json";
...
console.log(formatSmwRatingModelReport(model));

await mkdir(dirname(output), { recursive: true });
await writeFile(output, `${JSON.stringify(model, null, 2)}\n`, "utf8");
console.log(`\nWrote SMW rating model: ${output}`);
