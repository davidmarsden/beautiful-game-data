...
if (!inputPath || !outputPath) {
  console.error("Usage: node scripts/filter-transfermarkt-refresh.js --input=raw.json --output=accepted.json [--report=report.json]");
  process.exit(1);
}
...
for (const row of rows) {
  ...
  if (!rules.include_zero_market_value && !(number(row.market_value_eur) > 0)) reasons.push("zero_value_excluded");
  if (reasons.length) rejected.push({ transfermarkt_id: id, player_name: name, age, reasons });
...
console.log(JSON.stringify({ scanned: report.scanned, accepted: report.accepted, rejected: report.rejected, rejection_counts: report.rejection_counts }, null, 2));
