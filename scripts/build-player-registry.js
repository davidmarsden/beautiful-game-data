...
function tbgIdFromTransfermarktId(id) {
  return `tbg-tm-${String(id).padStart(8, "0")}`;
}
...
function registryRecordFromTransfermarkt(row, existing = null) {
  ...
  const aliases = unique([
    ...
    row.short_name,
    row.nickname
  ].map((value) => String(value ?? "").trim()).filter((value) => value && value !== displayName));
...
function mergeRegistry(existingRows, transfermarktRows) {
  ...
  const byTbgId = new Map();

  for (const row of existingRows) {
    if (row.transfermarkt_id) byTmId.set(String(row.transfermarkt_id), row);
    if (row.tbg_player_id) byTbgId.set(String(row.tbg_player_id), row);
  }
  ...
  for (const tmRow of transfermarktRows) {
    ...
    if (!record) continue;
    touched.add(record.tbg_player_id);
    merged.push(record);
  }

  for (const row of existingRows) {
    if (touched.has(row.tbg_player_id)) continue;
    merged.push({ ...row, status: row.status || "inactive" });
  }
